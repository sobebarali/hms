...
describe("POST /api/dispensing/:prescriptionId/dispense - Validation errors", () => {
	...
	it("returns 400 when medicines array is missing", async () => {
	...
	it("returns 400 when dispensedQuantity is missing", async () => {
		const response = await request(app)
			.post(`/api/dispensing/${prescriptionId}/dispense`)
			.set("Authorization", `Bearer ${accessToken}`)
			.send({
	...
	it("returns 400 when dispensedQuantity is negative", async () => {
		const response = await request(app)
			.post(`/api/dispensing/${prescriptionId}/dispense`)
			.set("Authorization", `Bearer ${accessToken}`)
			.send({
