...
describe("POST /api/dispensing/:prescriptionId/dispense - Medicine not in prescription", () => {
	...
	it("returns 400 when trying to dispense medicine not in prescription", async () => {
		const nonExistentMedicineId = uuidv4();
		const response = await request(app)
			...
			.send({
				medicines: [
					{
						medicineId: nonExistentMedicineId,
