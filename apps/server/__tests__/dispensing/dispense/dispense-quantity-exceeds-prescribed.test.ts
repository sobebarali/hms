...
describe("POST /api/dispensing/:prescriptionId/dispense - Quantity exceeds prescribed", () => {
			...
			medicines: [
				{
					...
					duration: "5 days",
					quantity: 15, // Prescribed quantity is 15
					dispensed: false,
				},
			],
			status: "DISPENSING",
	...
	it("returns 400 when dispensed quantity exceeds prescribed quantity", async () => {
		const response = await request(app)
			...
			.send({
				medicines: [
					{
						medicineId,
						dispensedQuantity: 100, // Trying to dispense more than prescribed
					},
				],
			});
