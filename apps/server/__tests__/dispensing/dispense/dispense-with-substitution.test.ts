...
describe("POST /api/dispensing/:prescriptionId/dispense - With substitution", () => {
	...
	it("dispenses with substitution info saved", async () => {
				...
				medicines: [
					{
						...
						substituted: true,
						substituteNote: "Substituted with generic equivalent",
					},
				],
			});
		...
		// Verify substitution info in database
		...
		expect(medicine?.substituted).toBe(true);
		expect(medicine?.substituteNote).toBe(
			"Substituted with generic equivalent",
		);
	});
});
