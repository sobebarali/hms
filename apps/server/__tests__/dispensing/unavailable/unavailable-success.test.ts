...
describe("POST /api/dispensing/:prescriptionId/unavailable - Mark unavailable success", () => {
	...
	it("marks medicine as unavailable with reason", async () => {
		...
		expect(response.body).toHaveProperty("medicineId", medicineId);
		expect(response.body).toHaveProperty("status", "UNAVAILABLE");
		expect(response.body).toHaveProperty("reason", "Out of stock");
		...
		expect(medicine?.status).toBe("UNAVAILABLE");
	});
});
