...
describe("POST /api/dispensing/:prescriptionId/complete - With counseling", () => {
	...
	beforeAll(async () => {
		...
		await Dispensing.create({
			...
			startedAt: new Date(),
			medicines: [{ medicineId, dispensedQuantity: 15, status: "DISPENSED" }],
			createdAt: new Date(),
			updatedAt: new Date(),
		});
	}, 30000);
	...
	it("completes with notes and patient counseling flag", async () => {
		const response = await request(app)
			.post(`/api/dispensing/${prescriptionId}/complete`)
			.set("Authorization", `Bearer ${accessToken}`)
			.send({
				notes: "Patient advised to take after meals",
		...
		// Verify counseling info saved in database
		const dispensing = await Dispensing.findOne({ prescriptionId }).lean();
		expect(dispensing?.notes).toBe("Patient advised to take after meals");
		expect(dispensing?.patientCounseled).toBe(true);
	});
});
