...
describe("POST /api/dispensing/:prescriptionId/complete - Pending medicines", () => {
	...
	beforeAll(async () => {
		...
		// Create dispensing with some medicines still PENDING
		await Dispensing.create({
			...
			medicines: [
				...
				{ medicineId: medicineId2, dispensedQuantity: 0, status: "PENDING" }, // Still pending
			],
			createdAt: new Date(),
			updatedAt: new Date(),
		});
	...
	it("returns 400 when some medicines are still pending", async () => {
