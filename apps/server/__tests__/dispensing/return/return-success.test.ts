...
describe("POST /api/dispensing/:prescriptionId/return - Return to queue success", () => {
	...
	beforeAll(async () => {
		context = await createAuthTestContext({
			rolePermissions: ["DISPENSING:UPDATE", "DISPENSING:READ"],
		...
		// Create dispensing with NO medicines dispensed yet
	...
	it("returns prescription to queue successfully", async () => {
		const response = await request(app)
			.post(`/api/dispensing/${prescriptionId}/return`)
			.set("Authorization", `Bearer ${accessToken}`)
			.send({
				reason: "Need to verify dosage with doctor",
			});
