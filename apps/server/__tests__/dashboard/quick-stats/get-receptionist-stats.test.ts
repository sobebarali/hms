...
describe("GET /api/dashboard/quick-stats - Receptionist stats", () => {
	...
	beforeAll(async () => {
		...
		await Appointment.create({
			...
			appointmentNumber: `APT-REC-${context.uniqueId}-1`,
			patientId: uuidv4(),
			doctorId: uuidv4(),
		...
		appointmentIds.push(confirmedId);
		...
		await Appointment.create({
			_id: confirmedId,
			tenantId: context.hospitalId,
			appointmentNumber: `APT-REC-${context.uniqueId}-2`,
			patientId: uuidv4(),
			doctorId: uuidv4(),
			departmentId: context.departmentId,
			date: today,
	...
	it("returns quick stats with pending tasks for receptionist", async () => {
