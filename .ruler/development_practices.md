...
- Never create unnecessary standalone documentation files
...
Follow this order when creating endpoints: **Validation → Repository → Service → Controller → Routes**

### Layer Responsibilities
- **Validation**: Zod schema (runtime), Input type (inferred from Zod), Output interface (manually defined)
- **Repository**: Database operations only, no business logic
- **Shared Repository**: Reusable lookups (`findById`, `findByEmail`) in `shared.{domain}.repository.ts`
- **Service**: Business logic, imports from repositories, throws typed errors
- **Controller**: HTTP handling only, calls service, returns response
- **Routes**: Registers endpoints with middleware

### Import Rules
- Services import shared lookups from `shared.{domain}.repository.ts`
- Cross-domain imports allowed for shared repositories only
- Endpoint repositories export only endpoint-specific operations
- Never re-export shared functions from endpoint repositories

### Implementation Checklist
- [ ] Validation: Create schema with Input/Output types
- [ ] Repository: Create database operations
- [ ] Service: Create business logic
- [ ] Controller: Create HTTP handler
- [ ] Routes: Register route with middleware
- [ ] Tests: Write tests in `__tests__/{domain}/{endpoint}/`
- [ ] Documentation: Update `apps/docs/` if needed
